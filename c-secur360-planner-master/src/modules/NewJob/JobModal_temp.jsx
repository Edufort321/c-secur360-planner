...
export function JobModal({
    ...
    const generateHierarchicalOptions = (excludeId = null, existingDeps = []) => {
        ...
        const renderHierarchicalOptions = (parentId = null, level = 0) => {
            const results = [];
            const currentLevelSteps = availableSteps
                .filter(etape => etape.parentId === parentId)
                .sort((a, b) => (a.order || 0) - (b.order || 0));

            currentLevelSteps.forEach(etape => {
                ...
                const childOptions = renderHierarchicalOptions(etape.id, level + 1);
    ...
    const generateHierarchicalCheckboxes = (selectedStep) => {
        ...
        const renderHierarchicalCheckboxes = (parentId = null, level = 0) => {
            ...
            currentLevelSteps.forEach(etape => {
                ...
                // Ajouter récursivement les enfants
                const childCheckboxes = renderHierarchicalCheckboxes(etape.id, level + 1);
                results.push(...childCheckboxes);
            });
